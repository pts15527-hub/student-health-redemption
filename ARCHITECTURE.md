...
```txt
保健食品
[新增領取紀錄] [更新未實現] [查看未實現] [返回]
```

保健食品採用少量選單 + 文字輸入，不把全部商品列成按鈕，避免 LINE 訊息過長。

入口分工：

- `新增領取紀錄`：建立正式扣組紀錄，也就是「本次扣了幾組、應領哪些商品」。
- `更新未實現`：記錄已扣組紀錄後，後續實際又領了哪些盒數。
- `查看未實現`：只列出「已扣組但尚未完整領取」的品項，不列出剩餘總組數可兌換的所有商品。
- `返回`：回到學生管理選單。

`未實現進度` 定義：

- 已扣除權益，但尚未完整交付的保健食品內容。
- 它不是剩餘組數，也不是可兌換商品清單。
- 剩餘組數回答「尚未使用的權益還有幾組」。
- 領取紀錄回答「哪一天正式扣了幾組」。
- 未實現進度回答「已扣組的內容裡，哪些品項還沒領完」。
...
B群｜來源 7/1｜應領 5盒｜已領 3盒｜尚可領 2盒
白賦美｜來源 7/8｜應領 5盒｜已領 4盒｜尚可領 1盒
...
- 新增領取紀錄確認後，系統可依 `redemption_record_items` 建立對應的未實現追蹤項目；預設可視為「尚未完整實現」，後續由 `更新未實現` 或管理介面補登實際領取事件。
...
| `/records` 保健食品領取紀錄 | 上半部為單筆領取紀錄，下半部為未實現進度 | 單筆紀錄顯示扣除組數；未實現進度顯示已扣組但尚未完整領取的品項 |
...
未實現進度只顯示已扣組但尚未完整領取的內容：

- 不顯示尚未使用的剩餘 15 組可以換哪些商品。
- 不顯示已完整領取且沒有待追蹤的品項，除非進入歷史明細。
- 學生端不可編輯，只能查看。
...
| `redemption_fulfillment_items` | 下一階段候選：每筆已扣組紀錄對應的未實現追蹤項目 |
| `redemption_fulfillment_events` | 下一階段候選：每次實際領取或補領事件 |
...
| `redemption_fulfillment_items` | 後續由領取紀錄自動建立，或由管理操作補登 |
| `redemption_fulfillment_events` | 後續由 LINE Bot `更新未實現` 或管理介面建立 |
