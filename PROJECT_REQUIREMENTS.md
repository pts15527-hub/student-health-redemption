...
- `新增領取紀錄`：建立正式扣組紀錄，記錄本次扣了幾組與應領商品。
- `更新未實現`：記錄已扣組紀錄後，後續實際又領了哪些盒數。
- `查看未實現`：只整理已扣組但尚未完整領取的項目，不列出剩餘組數可換的所有商品。

`未實現進度` 定義：

- 已扣除權益，但尚未完整交付的保健食品內容。
- 它不是剩餘組數，也不是可兌換商品清單。
- 學生端用 `/s/[share_token]/records` 呈現，上半部為單筆領取紀錄，下半部為未實現進度。
...
- 新增領取紀錄確認後，下一階段可依應領商品建立未實現追蹤項目。
- `更新未實現` 格式暫定為日期 + 商品盒數，例如 `7/15 B群 2盒`，實作前需再確認如何對應來源領取紀錄。
